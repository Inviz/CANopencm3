...
CO_ReturnError_t CO_flash_read_all_entries(CO_storage_entry_t *entries, uint8_t entriesCount) {
    ...
    for (uint16_t i = 0; i < entriesCount; i++) {
        ...
        if (entry->addrFlash != NULL) {
            CO_storage_flash_record_t *record = (CO_storage_flash_record_t *)(entry->addrFlash);
            entry->len = record->len;
            // write value to memory location
            memcpy(entry->addr, (void *) (entry->addrFlash + CO_STORAGE_FLASH_HEADER_SIZE), entry->len);
        }
    }
...
static CO_ReturnError_t CO_flash_compact_storage(CO_storage_t *storage) {
    ...
    for (uint16_t i = 0; i < storage->entriesCount; i++) {
        ...
        for (uint16_t entry_bytes_correct = 0; entry_bytes_correct < entry->len;) {
            uint16_t page_bytes_left =
                CO_STORAGE_FLASH_PAGE_SIZE - ((uint32_t)last_valid_address % CO_STORAGE_FLASH_PAGE_SIZE);
    ...
    for (void *dirty_page_address = first_dirty_page_address; dirty_page_address < CO_STORAGE_FLASH_OOB_ADDRESS;
    ...
    for (uint16_t i = first_invalid_entry; i < storage->entriesCount; i++) {
        ...
        for (uint16_t entry_bytes_correct = 0; entry_bytes_correct < entry->len;) {
            uint16_t page_bytes_left =
                CO_STORAGE_FLASH_PAGE_SIZE - ((uint32_t)current_writing_address % CO_STORAGE_FLASH_PAGE_SIZE);
            uint16_t page_bytes_used_by_entry = entry->len > page_bytes_left ? page_bytes_left : entry->len;
...
CO_ReturnError_t CO_flash_write_entry(CO_storage_entry_t *entry) {
    ...
    return CO_flash_write(entry->addrFlash, (uint8_t *)&storage_record, CO_STORAGE_FLASH_HEADER_SIZE) ||
           CO_flash_write(entry->addrFlash + CO_STORAGE_FLASH_HEADER_SIZE, entry->addr, entry->len);
}
