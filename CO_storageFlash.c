...
#define CO_STORAGE_FLASH_BASE_ADDRESS (void *) FLASH_BASE + CO_STORAGE_FLASH_DEVICE_SIZE - CO_STORAGE_FLASH_SIZE
#define CO_STORAGE_FLASH_OOB_ADDRESS (void *) CO_STORAGE_FLASH_BASE_ADDRESS + CO_STORAGE_FLASH_DEVICE_SIZE
...
CO_ReturnError_t CO_flash_read_all_entries(CO_storage_entry_t *entries, uint8_t entriesCount)
{
	...
	{
		CO_storage_flash_record_t *record = (CO_storage_flash_record_t*) current_record_address;
		...
		entry->addrFlash = record->len == 0 ? NULL : current_record_address + CO_STORAGE_FLASH_HEADER_SIZE;
	...
	for (uint16_t i = 0; i < entriesCount; i++)
	{
		...
		{
			CO_storage_flash_record_t *record = (CO_storage_flash_record_t*) (current_record_address - CO_STORAGE_FLASH_HEADER_SIZE);
...
static CO_ReturnError_t CO_flash_compact_storage(CO_storage_t *storage)
{
	...
	for (uint16_t i = 0; i < storage->entriesCount; i++)
	{
		...
		CO_storage_flash_record_t storage_record = {entry->index, CO_flash_generate_crc(entry), entry->len};

		// Check if meta record is valid
		// Worst edge case: First invalid meta record is split between two pages,
		// while only part on second page is is dirty: both pages will be invalidated
		if (memcmp(last_valid_address, &storage_record, sizeof(storage_record)) != 0)
		{
		...
		{
			uint16_t page_bytes_left = CO_STORAGE_FLASH_PAGE_SIZE - ((uint32_t) last_valid_address % CO_STORAGE_FLASH_PAGE_SIZE);
			uint16_t page_bytes_used_by_entry = entry->len > page_bytes_left ? page_bytes_left : entry->len;

			// new data is too big to fit the storage, have to abort
			if (last_valid_address + page_bytes_used_by_entry >= CO_STORAGE_FLASH_OOB_ADDRESS)
			{
				return CO_ERROR_OUT_OF_MEMORY;
			}

			// skip all valid segments in case value intersects page boundary
			if (memcmp(last_valid_address, entry->addr + entry_bytes_correct, page_bytes_used_by_entry) == 0)
			{
		...
		// all entries are stored and memory is already compacted, changes are unnecessary
	...
	first_dirty_page_address = last_valid_address - ((uint32_t) last_valid_address % CO_STORAGE_FLASH_PAGE_SIZE);
	...
	for (uint16_t i = first_invalid_entry; i < storage->entriesCount; i++)
	{
		...
		if (current_writing_address + CO_STORAGE_FLASH_HEADER_SIZE > last_valid_address)
		{
			CO_storage_flash_record_t storage_record = {CO_flash_generate_crc(entry), entry->index, entry->len};
			memcpy(current_writing_address, &storage_record, CO_STORAGE_FLASH_HEADER_SIZE);
			current_writing_address += CO_STORAGE_FLASH_HEADER_SIZE;
		}

		for (uint16_t entry_bytes_correct = 0; entry_bytes_correct < entry->len;)
		{
			uint16_t page_bytes_left = CO_STORAGE_FLASH_PAGE_SIZE - ((uint32_t) current_writing_address % CO_STORAGE_FLASH_PAGE_SIZE);
			uint16_t page_bytes_used_by_entry = entry->len > page_bytes_left ? page_bytes_left : entry->len;

			// first dirty entry may have valid segments in case it crosses page boundary
			if (current_writing_address + page_bytes_used_by_entry > last_valid_address)
			{
				memcpy((void *) current_writing_address, entry->addr + entry_bytes_correct, page_bytes_used_by_entry);
			}
			entry_bytes_correct += page_bytes_used_by_entry;
...
{
	// if there's no more room to append entry to, memory has to be compacted and that will write the entry
	if (current_record_address + entry->len + CO_STORAGE_FLASH_HEADER_SIZE >= CO_STORAGE_FLASH_OOB_ADDRESS)
	{
	...
	CO_storage_flash_record_t storage_record = {CO_flash_generate_crc(entry), entry->index, entry->len};
	if (memcmp(entry->addrFlash, &storage_record, CO_STORAGE_FLASH_HEADER_SIZE) == 0 &&
			memcmp(entry->addrFlash + sizeof(storage_record), entry->addr, entry->len) == 0)
	{
		return CO_ERROR_NO;
	}
	...
	CO_ReturnError_t status = CO_flash_write(entry->addrFlash, (uint8_t *) &storage_record, CO_STORAGE_FLASH_HEADER_SIZE);
	if (status != CO_ERROR_NO)
		return status;
	status = CO_flash_write(entry->addrFlash + CO_STORAGE_FLASH_HEADER_SIZE, entry->addr, entry->len);
	if (status != CO_ERROR_NO)
		return status;
...
{
	CO_storage_flash_record_t storage_record = {CO_flash_generate_crc(entry), entry->index, 0};
